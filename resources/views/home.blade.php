                ...
                @auth
                    ...
                    @if (auth()->user()->role === 'peminjam')
                        <a href="{{ route('peminjaman.index') }}"
                            class="bg-cyan-600 hover:bg-cyan-700 text-white px-3 py-1.5 rounded-lg text-sm font-medium transition">
                            Riwayat Saya
                        </a>
                    @else
                        <a href="{{ auth()->user()->role === 'petugas' ? '/q' : '/admin' }}"
                            class="bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-lg text-sm font-medium transition">
                            {{ auth()->user()->role === 'petugas' ? 'Dashboard Petugas' : 'Dashboard Admin' }}
                        </a>
                    @endif
                    ...
                    <form action="{{ route('logout') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit"
                            class="bg-red-500 hover:bg-red-600 text-white px-3 py-1.5 rounded-lg text-sm font-medium transition">
                            Logout
                        </button>
                    </form>
                @else
            ...
            @foreach ($tools as $tool)
                <div class="bg-white rounded-xl shadow-md p-6 flex flex-col justify-between border border-gray-100">
                    <div>
                        ...
                        @else
                            <div
                                class="w-full h-48 bg-gray-100 rounded-lg mb-4 flex items-center justify-center text-sm text-gray-400">
                                Tidak ada gambar
                            </div>
                        @endif
                        ...
                        <div class="space-y-1 mb-4 text-sm text-gray-600">
                            ...
                            <p>
                                ...
                                <span
                                    class="{{ $tool->stock > 0 ? 'text-green-600 font-bold' : 'text-red-500 font-bold' }}">
                                    {{ $tool->stock }} unit
                                </span>
                            </p>
                        </div>
                        ...
                        @auth
                            ...
                            @else
                                <div class="bg-gray-100 p-3 rounded-lg text-center text-xs text-gray-600">
                                    Akun <span class="font-semibold capitalize">{{ auth()->user()->role }}</span> tidak
                                    dapat meminjam alat.
                                </div>
                            @endif
                        @else
                            <a href="{{ route('login') }}"
                                class="block text-center w-full bg-amber-500 hover:bg-amber-600 text-white font-medium py-2 px-4 rounded-lg text-sm transition shadow">
